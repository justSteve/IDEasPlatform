...
3. If no, decide: does this work need a bead?
   - **Create one yourself** if the work is non-trivial (multi-file changes, new features, architectural decisions, anything you would want to track).
   - **Proceed without one** if the work is minor housekeeping (fixing a typo, updating a status field, small config tweak).
   - When in doubt, create the bead. It is cheap and the audit trail is valuable.
...
When you create a bead yourself, append it to `.beads/issues.jsonl` following the standard format. Use your judgment on `type` (epic vs task) and write a clear `description`. You own this decision — do not defer to the operator on bead creation, sub-task breakdown, or ID assignment.
